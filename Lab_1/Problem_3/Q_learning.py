...
class Robber:
    ...
    def get_optimal_action_Q(self, Q, eps, s):
        ...
        """
        enc_s, _, _, _ = self.get_enc()
        key = ((s['A'][0], s['A'][1]), (s['B'][0], s['B'][1]))
        s_index = enc_s[key]
    ...
    def get_exp_sarsa(self,Q,eps):
        ...
        Returns (st, at, rt, st+1)
        """
        _, _, _, dec_a = self.get_enc()
        s = copy.deepcopy(self.s)
        a_i = self.get_optimal_action_Q(Q,eps,s)
        a = dec_a[a_i]
        r = self.reward(self.s)
        self.update_state(a, minotaor = False)
        self.update_state(a, minotaor = True)
        s_next = copy.deepcopy(self.s)
        a_next = self.get_optimal_action_Q(Q,eps,s_next)
        return {'s':s, 'a':a_i, 'r':r, 's_next':s_next, 'a_next':a_next}
    ...
    def Sarsa(self, eps):
        ...
        for i in range(10000000):
            ...
            #print(exp['s'], decoder_a[exp['a']], exp['s_next'], decoder_a[exp['a']])
            key = ((exp['s']['A'][0], exp['s']['A'][1]), (exp['s']['B'][0], exp['s']['B'][1]))
            key_next = ((exp['s_next']['A'][0], exp['s_next']['A'][1]), (exp['s_next']['B'][0], exp['s_next']['B'][1]))
            i_s = encoder_s[key]
            i_s_next = encoder_s[key_next]
            ...
            if i % 10000 == 0:
                print(np.reshape(Q[15*16:16*16, 2], (4,4)))
            if i_s == 240 and i_a == 0:
                print('up', Q[240,0])
                up.append(Q[240,0])

            if i_s == 240 and i_a == 1:
                print('right', Q[240,1])


                right.append(Q[240,1])
            if i_s == 240 and i_a == 2:
                print('down', Q[240,2])
                down.append(Q[240,2])

            if i_s == 240 and i_a == 3:
                print('left', Q[240,3])
                left.append(Q[240,3])
            if i_s == 240 and i_a == 4:
                print('still', Q[240,4])
                still.append(Q[240,4])
        with open('up1.pkl', 'wb') as f:
            pickle.dump(up, f)
        with open('right1.pkl', 'wb') as f:
            pickle.dump(right, f)
        with open('down1.pkl', 'wb') as f:
            pickle.dump(down, f)
        with open('left1.pkl', 'wb') as f:
            pickle.dump(left, f)
        with open('still1.pkl', 'wb') as f:
            pickle.dump(still, f)
